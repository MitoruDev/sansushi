"use client";

import { motion, useReducedMotion } from "framer-motion";

const sushiPath =
  "M10.39,29.39l-0.18,0.09c0.04,0.09,0.07,0.18,0.09,0.28c3.62,15.97,11.21,17.62,24.21,20.43 c1.75,0.38,3.6,0.78,5.63,1.26l35.57,8.34c5.4,1.05,10.62,1.23,15.42,0.32c3.25-0.62,6.32-1.73,9.12-3.4 c-9.26,0.05-15.94-2.2-25.03-5.99c-14.65-6.1-24.81-14.26-36.81-19.42C29.55,27.49,20.37,25.76,10.39,29.39L10.39,29.39z M6.74,30.84c-3.83,1.18-6.44,0.59-6.73-3.31c-0.23-4.28,1.99-8.43,6.6-12.44c28.07-24.45,75.2-18.11,97.38,11.34 c6.8,9.04,11.16,17.03,18.89,26.42c-3.07,1.6-6.18,2.69-9.36,3.15c-0.83,0.12-4.01,0.32-7.39,0.5c-0.08,0.42-0.3,0.82-0.66,1.1 c-4.04,3.23-8.66,5.23-13.65,6.17c-5.28,1-10.97,0.82-16.82-0.32L39.3,55.06c-1.89-0.44-3.78-0.85-5.58-1.24 C19.34,50.71,10.91,48.87,6.74,30.84L6.74,30.84z M43,11.26c6.66-2.53,13.44-3.92,18.59-3.22c-3.75,2.4-7.81,4.56-12.6,6.46 c0.01,4.83-0.58,10.76-1.66,15.53C46.43,23.74,45.14,16.48,43,11.26L43,11.26z M82.69,29.94c6.86-1.92,13.74-2.69,18.81-1.53 c-3.95,2.06-8.19,3.83-13.13,5.29c-0.43,4.81-1.55,10.66-3.06,15.32C84.98,42.68,84.35,35.34,82.69,29.94L82.69,29.94z M63.91,18.89c6.86-1.92,13.74-2.69,18.81-1.53c-3.95,2.06-8.19,3.83-13.13,5.29c-0.43,4.81-1.55,10.66-3.06,15.32 C66.19,31.63,65.56,24.29,63.91,18.89L63.91,18.89z";

function SushiSvg() {
  return (
    <svg
      viewBox="0 0 122.88 64.41"
      className="h-6 w-auto"
      fill="currentColor"
      aria-hidden
    >
      <path fillRule="evenodd" clipRule="evenodd" d={sushiPath} />
    </svg>
  );
}

export function FooterSushiMicro() {
  const reduceMotion = useReducedMotion();

  if (reduceMotion) {
    return (
      <span
        className="hidden justify-center text-primary-on-dark md:flex"
        aria-hidden
      >
        <SushiSvg />
      </span>
    );
  }

  return (
    <motion.span
      className="hidden cursor-default justify-center text-primary-on-dark md:flex"
      aria-hidden
      whileHover={{ scale: 1.08, rotate: [0, -6, 6, -3, 0] }}
      transition={{
        duration: 0.42,
        ease: [0.34, 1.56, 0.64, 1],
      }}
    >
      <SushiSvg />
    </motion.span>
  );
}
